...
arima.search <- get.best.arima(co2, maxord=c(3,2,3,3,2,3))
```
...
**Our best `sarima` model can be expressed as below**    
  $$(1-B)(1-B)^{12} x_t = (1+\theta_1B + \theta_{12}B^{12} + \theta_{13}B^{13} )w_t $$
**After solving the above equation with the results from R model, we get** 
      $x_t = x_{t-1} + ($ `r co2.sarima.2$coef[2]` $) * x_{t-12}$ + $w_t$ + $($ `r co2.sarima.2$coef[1] ` $) * w_{t-1}$ +  $($ `r co2.sarima.2$coef[3]`  $) * w_{t-12}$  +  $($ `r co2.sarima.2$coef[4]`  $) * w_{t-13}$       
    where $x_{t-12}$ represents 12th lag of time series and $x_{t-1}$ is the results of first difference of time series i.e.  $x^{1}_t = x_t - x_{t-1}$     
...
newxreg = rep(c(max(time(co2)^2)),each=284)
co2.forecast <- forecast(co2.sarima.2,  newxreg = newxreg, 284)
...
```{r}
co2.noaa.training <-  co2_noaa_monthly_df %>% filter(year <= 2019) 
co2.6months.data <-  co2.noaa.training %>% filter(year == 2019 & month > 6)
co2.noaa.training <- co2.noaa.training %>% anti_join(co2.6months.data, by=c("year", "month"))
co2.noaa.test <- co2_noaa_monthly_df %>% filter(year > 2019)
co2.noaa.test <- co2.noaa.test %>% union_all(co2.6months.data)

co2.noaa.training.ts = ts(co2.noaa.training$ppm_month_avg, start=c(1974,5), 
                       frequency=12)

co2.noaa.test.ts = ts(co2.noaa.test$ppm_month_avg, start=c(2019,7), 
                       frequency=12)
...
```{r}
noaa.arima.search <- get.best.arima(co2.noaa.training.ts, maxord=c(3,2,3,3,2,3))
best10.arima <- head(arima.search[with(noaa.arima.search, order(AICc)),], n=10)
row.names(best10.arima) <- NULL
kable(best10.arima, caption='Top 10 Models.')
```
```{r}
