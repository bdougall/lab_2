...
```{r}
co2.ppm = c(diff(co2))
noaa.prior1998.diff = c(diff(noaa.prior1998.ts))
forecasted.diff = c(diff(forecasted.ts))

max_ln <- max(c(length(co2.diff), length(noaa.prior1998.diff), 
      length(forecasted.diff)) )

combined_diff<- data.frame(co2.diff = c(co2.diff,rep(NA, max_ln - length(co2.diff))),
                      noaa.prior1998.diff = c(noaa.prior1998.diff,
                          rep(NA, max_ln - length(noaa.prior1998.diff))),
                      forecasted.diff = c(forecasted.diff,
                          rep(NA, max_ln - length(forecasted.diff))))


combined_diff$id <- row.names(combined_diff)

ggplot(data=combined_diff, aes(x=id)) +
  geom_line(aes(y=co2.diff))
