...
stt.preds.start <- ts(predict(co2.ts.lm.stt, new.dat), st=1974, fr=12)
...
co2.sarima.2 <- arima(co2, order=c(p,d,q),
                      ...
                      method="CSS")
co2.sarima.2.account_quad=arima(co2-stt.preds.start, order=c(p,d,q),
                      seasonal = list(order=c(P,D,Q)), 
                      method="CSS")
...
co2.forecast.account_quad<-forecast (co2.sarima.2.account_quad,284)
co2_forecast_ts <- co2.forecast[4]$mean
co2_forecast_ts <-co2.forecast.account_quad[4]$mean
...
while(num_missing>0) {
 ...
 #0 1 1 1 1 2 ? (this is monthly)
 # 1 AR (acf)
 # 1 difference (smallest?)
 # 1 MA (acf)
 # most parsimonious model? 1 1 2?
 best_model_params=get.best.arima.as.comp(input_arima_ts)
 best_model_params_order=best_model_params[c(1,2,3)]
 best_model_params_season=best_model_params[c(4,5,6)]
...
co2_weekly2 <- data.frame(lapply(co2_weekly_unmut, 
                  function(X) approxfun(seq_along(X), X)(seq_along(X))))
...
############################################
#co2_actuals_ts is from part 4
#co2_forecast_ts is from part 3 forecast
print(paste("dim of stt.preds.start is ",dim(stt.preds.start)))
print(paste("dim of co2_forecast_ts is ",dim(co2_forecast_ts)))
