...
**Our best `sarima` model can be expressed as below in the form backshift operator**    
  $$(1-\phi_1B)(1-\Phi_1B^{12} - -\Phi_2B^{13})(1-B)(1-B^{12}) x_t = (1+\theta_1B)  (\Theta_{12}B^{12} + \Theta_{13}B^{13} )w_t $$
...
**After solving for coefficients using R `arima` model, we get**   
    ...
    where $x_{t-12}$ and $x_{t-13}$represents 12th & 13th lag of time series, which comes from seasonal part of arima model. The $x_{t-1}$ is the results of first difference of time series i.e.  $x^{1}_t = x_t - x_{t-1}$     
...
arima.mod1 <- arima(train.ts, order = c(0,1,1), seas = list(order=c(0,1,1), 
...
**Our best `sarima` model can be expressed as below in the form backshift operator**    
  $$(1-\phi_1B)(1-B)(1-B^{12}) x_t = (1+\theta_1B)  (\Theta_{12}B^{12} )w_t $$
    
$(1-\phi_1B)$ represents auto regressive term,  $(1+\theta_1B)$ represents moving average term and  $(\Theta_{12}B^{12})$ represents seasonal moving average of `arima` model. $w_t$ represents white noise of the time series.$$

  
**After solving for coefficients using R `arima` model, we get**   
      $x_t = x_{t-1}$ +  $($ `r co2.sarima.2$coef[2] ` $) * x_{t-1}$ + $w_t$ + $($ `r co2.sarima.2$coef[2] ` $) * w_{t-1}$ +  $($ `r co2.sarima.2$coef[3]`  $) * w_{t-12}$         
        
    where  $x_{t-1}$ is the results of first difference of time series i.e.  $x^{1}_t = x_t - x_{t-1}$ and second $x_{t-1}$ is the result of auto regressive term.      
    $w_t$ is the white noise from the current time step, $w_{t-1}$ is white noise from the previous time step, which is the result of the AR moving average. $w_{t-12}$ is the white noise from 12 steps before (seasonal) current time step. This is the result of the seasonal moving average component of our model.
...
ts.plot(train.ts, co2.sarima.3$fit, lty=1:2,
        col=c("navy", "blue"),
        ylab="CO2 (ppm)",
        main="SARIMA(0,1,1,0,1,1) Forecasts vs. In-Sample Monthly CO2 Levels" 
        )
legend("topleft", legend=c("Actual", "Forecast"), col=c("navy", "blue"), lty=1:2)
```
...
ts.plot(test.ts, co2_forecast_ts24mo, lty=1:2,
        col=c("navy", "blue"),
        ylab="CO2 (ppm)",
        main="SARIMA(0,1,1,0,1,1) Forecasts vs. Out-of-Sample Monthly CO2 Levels" 
        )
legend("topleft", legend=c("Actual", "Forecast"), col=c("navy", "blue"), lty=1:2)
```
...
rmse(co2.sarima.3$residuals)
rmse(actuals_fore_diff2)
mae(co2.sarima.3$residuals)
mae(actuals_fore_diff2)
