...
as_tsibble(co2) %>%
  ...
  labs(title = "Non-Seasonal 1st Difference", y = "")
...
plot(co2.sarima$residuals, main = "SARIMA (0,1,2) (1,0,0)")
...
plot(plt.acf, main = "SARIMA (0,1,2) (1,0,0)")
plot(plt.pacf,main = "SARIMA (0,1,2) (1,0,0)")
...
Since the ACF and PACF plots still showed several strong autocorrelations, we proceeded to iterate over model parameters to see if we could improve the AIC score and create a model with residuals that better approximated white noise.
...
plot(residuals(co2.sarima.2))
acf(co2.sarima$residuals)
pacf(co2.sarima.2$residuals)
sarima2.aicc
Box.test(co2.sarima.2$residuals, type="Ljung-Box")
```
The AICc value is smaller than that of our initial model estimate, and the majority of ACF and PACF values fall within the $95\%$ confidence interval bounds for white noise. In addition, the model passes the Ljung-Box test, meaning that we reject the null hypothesis of correlation in the residuals.
...
Since this model has a lower AICc than our initial estimate, the residuals resemble white noise, and we have not found significant evidence of residual autocorrelation, we proceed with using this model in our forecast.
