...
plot(co2.deseasoned, 
    main = "Figure 2: Presence of CO2 after removing season", 
    xlab = "year", ylab = "CO2 ppm")

plot(co2.detrended, 
     main = "Figure 3: Presence of CO2 after removing trend", 
     xlab = "year", ylab = expression("CO2 ppm"), col = 'red', las= 1)

abline(h= 0)

plot(co2.diff, 
main = "Figure 4: Presence of CO2 after differencing", 
xlab = "year", ylab = expression("CO2 ppm"), col = 'red', las= 1)

abline(h= 0)

plot(co2.seasdiff, 
main = "Figure 5: Presence of CO2 after seasonal differencing", 
xlab = "year", ylab = expression("CO2 ppm"), col = 'red', las= 1)
abline(h= 0)

plot(co2.bothdiff, 
  main = "Figure 6: Presence of CO2 w/ non-seasonal & seasonal differencing", 
         xlab = "year", ylab = expression("CO2 ppm"), col = 'red', las= 1)
abline(h= 0)
...
From Figure 2: We see a clear upward trend in the mean of the presence of Co2 in the air.   
...
From Figure 6: Seasonality and trend are absent after one lag and 12 lags for the season. After seasonal and non-seasonal differencing, the series appears stationary with a relatively constant mean.
From Figure 7: Seasonality is apparent across months. For instance, notice the increase in CO2 from the first quarter to the end of the second quarter (month 6), and then the decline in CO2 until October. 
...
plot(plot.acf.diff, 
     main = "ACF CO2 Presence in air \n AR First differencing (1959 - 1997)", 
     xlab = "Year", ylab = "Co2 ppm", col="blue")
plot(plot.pacf.diff, 
     main = "PACF CO2 Presence in air \n AR first differencing (1959 - 1997)", 
     xlab = "Year", ylab = "Co2 ppm", col="red", cex.main=0.5)

plot(plot.acf.seasondiff, 
     main = "ACF CO2 Presence in air \n seasonal diff (1959 - 1997)", 
     xlab = "Year", ylab = "Co2 ppm", col="blue")
plot(plot.pacf.seasondiff, 
     main = "PACF CO2 Presence in air \n season difference (1959 - 1997)", 
     xlab = "Year", ylab = "Co2 ppm", col="red", cex.main=0.5)

plot(plot.acf.bothdiff, 
     main = "ACF CO2 Presence in air \n AR and seasonal differences", 
     xlab = "Year", ylab = "Co2 ppm", col="blue", cex.main=0.5)
plot(plot.pacf.bothdiff, 
     main = "PACF CO2 Presence in air \n AR and seasonal differences", 
     xlab = "Year", ylab = "Co2 ppm", col="red", cex.main=0.5)
```
  ...
  * ACF graph after performing auto regressive (AR) and seasonal differences looks closer to white noise ACF graph. Significant correlations at a 1 year lag suggests the need for a MA term.   
  * In the PACF with AR and seasonal differences plot, the significant negative correlations at 1 and 2 year lags suggest we should explore using a seasonal AR term in our model. 
...
qqPlot(co2.ts.lm.stt$residuals, 
    main = expression("Quadratic Time Trend Model w/ 12 Seasonal Components"))
plt.acf = acf(co2.ts.lm.stt$residuals, plot = FALSE)
plt.pacf = pacf(co2.ts.lm.stt$residuals, plot = FALSE)
plot(plt.acf, 
 main = expression("ACF - Quadratic Time Trend Model with Season terms"))
plot(plt.pacf, 
 main = expression("PACF - Quadratic Time Trend Model with Season terms"))
...
# The PACF chart has fewer repeated significant spikes at seasonal lags 
# than the ACF does # so we'll use it for the seasonal part of the model 
# in our initial estimate
# The PACF only a seasonal spike at a lag of 12 - (1,0,0). 
#Since we used the PACF for the seasonal part, we'll estimate 
#the non-seasonal with the ACF
# The first 2 autocorrelations in the ACF are significant, 
# so we'll estimate an MA(2)
...
co2.sarima.aicc <- -2 * co2.sarima$loglik 
                    + log(length(co2) + 1)*(length(co2.sarima$coef))
co2.sarima.aicc
...
Since the ACF and PACF plots still showed a few strong autocorrelations and the plot of the inverse unit roots showed values near unity, we proceeded to iterate over model parameters to see if we could improve the AIC score and create a model with residuals that better approximated white noise.
...
sarima2.aicc <- -2 *co2.sarima.2$loglik 
              + (log(length(co2))+1) * length(co2.sarima.2$coef)
...
plot(co2.forecast, 
     main = "Best SARIMA Model - CO2 Forecasts to Present Day")
```
...
ts.plot(co2_actuals_ts, co2_forecast_ts, lty=1:2,
        col=c("navy", "blue"),
        ylab="CO2 (ppm)",
        main="SARIMA(0,1,1,2,1,2) Forecasts vs. Actual CO2 Levels" 
        )
legend("topleft", legend=c("Actual", "Forecast"), 
       col=c("navy", "blue"), lty=1:2)
```
...
ts.plot(actuals_fore_diff, lty=2,
        col=c("blue"),
        ylab="CO2 (ppm)",
        main="Difference between Actual and Forecasted CO2 Levels" 
        )
```
...
pre98.df <- subset(co2_noaa_monthly_df, 
                   (year < 1998 & year > 1974))['ppm_month_avg']
...
legend("topleft", legend=c("Pre-1998", "Post-1998"), 
                          col=c("navy", "blue"), lty=1)
...
train.df <- subset(co2_noaa_monthly_df, 
                  (year < 2019 & month >= 1) | (year == 2019 & month <= 6))
test.df <- subset(co2_noaa_monthly_df, 
                  (year == 2019 & month > 6) | (year > 2019 & month >= 1))
...
plot(train.ts, main="Training Series: NOAA Monthly-Averaged Obsevations")

# Use additive decomposition, since the magnitude of the seasonal 
# fluctuation doesn't appear to vary  by time series level
...
After taking one non-seasonal difference of the monthly-averaged NOAA series, we noticed there were fewer repeated seasonal lags in the PACF (only 1 at 12), so we'll use the PACF to estimate the seasonal part of the model (1,0,0). Furthermore, there are 2 significant autocorrelations at lags 1 and 2 in the ACF, so we'll estimate (0,1,2).
...
# There are 2 significant autocorrelations at lags 1 and 2 in the ACF, 
# so we'll estimate (0,1,2)
arima.mod1 <- arima(train.ts, order = c(0,1,2), 
                    seas = list(order=c(1,0,0), 
...
sarima3.aicc <- -2 *co2.sarima.3$loglik 
                + (log(length(co2))+1) * length(co2.sarima.3$coef)
...
plot(co2.forecast.24mo, 
     main = "Best SARIMA Model - CO2 Forecasts")
```
...
ts.plot(train.ts, co2.sarima.3$fit, lty=1:2,
        col=c("navy", "blue"),
        ylab="CO2 (ppm)",
        main="SARIMA(1,1,1,0,1,1) Forecasts vs. In-Sample CO2 Levels" 
        )
legend("topleft", 
       legend=c("Actual", "Forecast"), col=c("navy", "blue"), lty=1:2)
```
...
ts.plot(test.ts, co2_forecast_ts24mo, lty=1:2,
        col=c("navy", "blue"),
        ylab="CO2 (ppm)",
        main="SARIMA(1,1,1,0,1,1) Forecast vs. Actual CO2 Levels" 
        )
...
time_point_est=as.yearmon(time(pt_data))[pt_first_idx_w450]

####################################
#get lower confidence interval data
lower_data=co2.forecast.2100$lower
#at the 95%  confidence level
lower_data_95=lower_data[,"95%"]
#find the first interval covering 450ppm
first_95_int_w450=which(lower_data_95>=450)[1]
time_lower_95=as.yearmon(time(co2.forecast.2100$lower))[first_95_int_w450]

```

The first time the point estimate reaches 450ppm is in `r time_point_est`.
Upon inspecting the forecast's confidence intervals' upper bound, we observe that the first time that it reaches 450ppm is in `r time_upper_95`, while the lower bound reaches 450ppm in `r time_lower_95`.
